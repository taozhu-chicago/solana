...
mod tests {
    ...
    fn test_cost_model_token_transaction() {
        ...
        let (program_execution_cost, _loaded_accounts_data_size_cost) =
            CostModel::get_estimated_execution_cost(&token_transaction, &FeatureSet::all_enabled());
        assert_eq!(
            DEFAULT_INSTRUCTION_COMPUTE_UNIT_LIMIT as u64,
            program_execution_cost
        );
        assert_eq!(0, CostModel::get_instructions_data_cost(&token_transaction));
    }
    ...
    fn test_cost_model_compute_budget_transaction() {
        ...
        assert_eq!(expected_cu_limit as u64, programs_execution_cost);
        assert_eq!(
            expected_loaded_accounts_cost,
            loaded_accounts_data_size_cost
        );
    ...
    fn test_cost_model_message_many_different_instructions() {
        ...
        let (programs_execution_cost, _loaded_accounts_data_size_cost) =
            CostModel::get_estimated_execution_cost(&tx, &FeatureSet::all_enabled());
        assert_eq!(expected_cost, programs_execution_cost);
        assert_eq!(0, CostModel::get_instructions_data_cost(&tx));
    }
    ...
    fn test_transaction_cost_with_mix_instruction_without_compute_budget() {
        ...
        let (programs_execution_cost, _loaded_accounts_data_size_cost) =
            CostModel::get_estimated_execution_cost(&transaction, &FeatureSet::all_enabled());
