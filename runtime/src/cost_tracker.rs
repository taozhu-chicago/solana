...
mod tests {
    ...
    fn test_cost_tracker_commit_and_cancel() {
        ...
        // assert the block cost includes both tx
        assert_eq!(cost1 + cost2, testee.block_cost);
        ...
        // assert tx2 cost will be remvoed from tracker if cancelled
        testee.cancel_transaction(&tx2, &tx_cost2);
        assert_eq!(cost1, testee.block_cost);

        // still can add tx2
        assert!(testee.try_add(&tx2, &tx_cost2).is_ok());
        // reached block limit, cannot add tx1 anymore
        assert!(testee.try_add(&tx1, &tx_cost1).is_err());
        // commit tx2, the block will have both tx1 and tx2
