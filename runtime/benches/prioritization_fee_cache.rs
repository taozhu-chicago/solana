...
fn process_transactions_multiple_slots() {
    ...
    let banks = vec![
        Arc::new(Bank::new_from_parent(&bank, &collector, 1)),
        Arc::new(Bank::new_from_parent(&bank, &collector, 2)),
        Arc::new(Bank::new_from_parent(&bank, &collector, 3)),
        Arc::new(Bank::new_from_parent(&bank, &collector, 4)),
        Arc::new(Bank::new_from_parent(&bank, &collector, 5)),
    ];
// */
...
fn bench_process_transactions_multiple_slots(bencher: &mut Bencher) {
    ...
    let bank = bank_forks.working_bank();
    let collector = solana_sdk::pubkey::new_rand();
    let banks = (1..=NUM_SLOTS).map(|n| Arc::new(Bank::new_from_parent(&bank, &collector, n as u64))).collect::<Vec<_>>();
